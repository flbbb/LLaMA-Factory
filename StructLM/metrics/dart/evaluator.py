...
def postprocess_text(preds, references_s, metric_name):
    ...
    references_s = [
        [reference.strip() for reference in references] for references in references_s
    ]

    if metric_name in [
        "metrics/dart/moverscore.py",
        "metrics/dart/meteor.py",
        "bleurt",
    ]:
...
class EvaluateTool(object):
    ...
    def evaluate(self, preds, golds, section):
        ...
        if section in ["train", "dev"]:
            ...
            metric_list = [
                "sacrebleu"
            ]  # , "metrics/dart/meteor.py", "metrics/dart/ter.py", "bertscore", "bleurt"]
        ...
        for metric_name in metric_list:
            ...
            processed_preds, processed_golds = postprocess_text(
                preds, references_s, metric_name
            )
            ...
            if metric_name == "bertscore":
                ...
                # processed_golds = [
                #     [gold.split() if gold is not None else None for gold in ref]
                #     for ref in processed_golds
                # ]
