        ...
        for sub_class_name, sub_class_entity_list in entity.items():
            if sub_class_name == "poi":
                for entity_item in sub_class_entity_list:
                    under_scored_entity_dict[str(entity_item["address"])] = str(
                        entity_item["address"]
                    ).replace(" ", "_")
                    under_scored_entity_dict[str(entity_item["poi"])] = str(
                        entity_item["poi"]
                    ).replace(" ", "_")
                    under_scored_entity_dict[str(entity_item["type"])] = str(
                        entity_item["type"]
                    ).replace(" ", "_")
            elif sub_class_name == "distance":
                for entity_item in sub_class_entity_list:
                    under_scored_entity_dict[str(entity_item) + " miles"] = (
                        str(entity_item) + " miles"
                    )
            elif sub_class_name == "temperature":
                for entity_item in sub_class_entity_list:
                    under_scored_entity_dict[str(entity_item) + "f"] = (
                        str(entity_item) + "f"
                    )
            else:
                for entity_item in sub_class_entity_list:
                    under_scored_entity_dict[str(entity_item)] = str(
                        entity_item
                    ).replace(" ", "_")
...
class EvaluateTool(object):
    ...
    def evaluate(self, preds, golds, section):
        ...
        matching_paths = glob.glob(
            f"{DATA_PATH}/downloads/extracted/*/kvret_entities.json"
        )
        ...
        for metric_name in metric_list:
            ...
            metric = evaluate.load(os.path.join(METRICS_PATH, metric_name))
