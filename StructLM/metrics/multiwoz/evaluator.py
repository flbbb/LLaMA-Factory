...
def postprocess_text(mode, preds, golds):
    ...
    for pred, gold in zip(preds, golds):
        ...
        if gold["turn_id"] not in predictions[dial_id]["turns"].keys():
            predictions[dial_id]["turns"][gold["turn_id"]] = {
                "turn_belief": cleaned_gold_belief,
                "pred_belief": [],
            }
        ...
        for pred_slot_value in pred.split(", "):
            ...
            if "not mentioned" in pred_bs or "none" in pred_bs:
                continue
            predictions[dial_id]["turns"][gold["turn_id"]]["pred_belief"].append(
                pred_bs
            )
...
class EvaluateTool(object):
    ...
    def evaluate(self, preds, golds, section):
        ...
        matching_paths = glob.glob(
            f"{DATA_PATH}/downloads/extracted/*/processed_data/ontology.json"
        )
