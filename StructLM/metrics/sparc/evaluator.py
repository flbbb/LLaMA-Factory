...
class EvaluateTool(object):
    ...
    def evaluate(self, preds, golds, section):
        ...
        matching_paths = glob.glob(f"{DATA_PATH}/downloads/extracted/*/sparc/database")
        ...
        for i in range(len(golds)):
            if (
                golds[i]["query"].lower()
                == "SELECT T1.id, T1.name FROM battle EXCEPT SELECT T1.id, T1.name FROM battle AS T1 JOIN ship AS T2 ON T1.id  =  T2.lost_in_battle WHERE T2.location  =  'English Channel'".lower()
            ):
                golds[i][
                    "query"
                ] = "SELECT id, name FROM battle EXCEPT SELECT T1.id, T1.name FROM battle AS T1 JOIN ship AS T2 ON T1.id  =  T2.lost_in_battle WHERE T2.location  =  'English Channel'"
