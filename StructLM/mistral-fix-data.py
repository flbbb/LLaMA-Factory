...
with open(f"{DATA_PATH}/processed/skginstruct_test_file_7b.json", "r") as f:
    data = json.load(f)
...
for i, each in tqdm(enumerate(data)):
    ...
    data[i]["formatted_input"] = prompt.format(
        instruction=instruction, input=struct_in, question=each["text_in"]
    )
...
with open(
    f"{DATA_PATH}/processed/skginstruct_test_file_mistral_examples.txt", "w"
) as f:
    f.write(printstr)


with open(f"{DATA_PATH}/processed/skginstruct_test_file_mistral.json", "w") as f:
